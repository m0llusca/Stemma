...
function buildTlsOptions(config: ParsedLdapsConfig) {
  const ca = [
    ...
    ...config.caFileRefs.map((ref) => readFileSync(/* turbopackIgnore: true */ resolveManagedTextRef(ref), "utf8"))
  ];
...
export async function syncActiveDirectoryLdapsProvider(input: {
  ...
  validateLdapsProviderConfigForSave({
    ...
    config: parseLdapsConfigRecord(input.provider.configJson)
  });
