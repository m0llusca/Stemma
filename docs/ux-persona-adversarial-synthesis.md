...
Leftovers (not fixed): catalog `readinessTone`; AGENT deep-link `/dashboard`; shell `useSearchParams` without layout Suspense; #24 `peer_quality:read` vs `!== SUPPORT_AGENT`.
